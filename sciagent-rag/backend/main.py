...
# 使用 rag_manager 内部的 db_manager，避免重复初始化
...
def generate_command(request: QuestionRequest):
    ...
    try:
        ...
        if is_valid:
            return {"status": "success", "command": parsed_command, "raw_command": command}
        else:
        ...
        while True:
            ...
            try:
                ...
                if message.get("type") == "command":
                    # 验证命令
                    is_valid, validate_message = command_validator.validate_command(message.get("data"))
